...
df <- read_csv("https://raw.githubusercontent.com/ucb-stat133/stat133-fall-2015/raw/master/data/tents1.csv")
```
...
```{r}
# fill in
avg_prices <- vector('numeric', 8)

brands <- levels(df$brand)
for (k in unique(df$brand)) {
  avg_prices[k] <- mean(df$price[df$brand == brands[k]])
}
avg_prices
